...
def now_utc_iso() -> str:
    # Avoid deprecated utcnow(); use tz-aware UTC
...
def save_to_airtable(payload: dict) -> tuple[bool, str]:
    ...
    # Flat fields + raw JSON for reliability
    ...
    fields = {
        ...
        "passengers_summary": pax_summary,
...
def airline_badge_svg(code: str, style_seed: int = 0) -> str:
    """
    Inline SVG badge to avoid CORS/network issues.
    Uses a pleasant gradient; deterministic per seed.
    """
...
# =========================
# Theme + UI polish (System-first, no "white on white" regressions)
# =========================
def apply_theme(mode: str):
    """
    System: use adaptive CSS that follows prefers-color-scheme.
    Light/Dark: force safe, complete overrides (including text color) so Render never fades out.
    """
    st.markdown(
        """
        <style>
          ...
          /* Cards (System-adaptive defaults) */
          .ow-hero {
            border-radius: 22px;
            padding: 22px 22px;
          ...
          /* SYSTEM: Dark scheme (very black, but readable) */
    ...
    if mode == "Light":
        # Force safe light mode INCLUDING text color (fixes Render fade / white-on-white)
        st.markdown(
            """
            <style>
              html, body, [class*="stApp"] { background: #f7f9fc !important; color: #0f172a !important; }
...
def _apply_pending_actions_before_widgets():
    """
    CRITICAL: Must run BEFORE widgets are created.
    ...
    if action == "swap":
        ...
        # Keep last search results; user wants them to remain
        st.rerun()
    ...
    if action == "clear":
        ...
        # Do NOT wipe last search automatically (user wants search highlight to remain)
        st.rerun()
...
def airport_selector_block(title: str, key_prefix: str, preselected: dict | None):
    st.markdown(f"<div class='ow-card'>", unsafe_allow_html=True)
    st.subheader(title)

    q_key = f"{key_prefix}_q"
    # Use session_state values as the source of truth for visible movement
...
if not origin_airport or not dest_airport:
    ...
    if cards:
        ...
        for idx, r in enumerate(cards):
            ...
            with left:
                st.markdown(airline_badge_svg(r.get("airline_code", "XX"), style_seed=r.get("style_seed", idx)), unsafe_allow_html=True)
            with mid:
    ...
    # Always render the last booking section if it exists (so booking persists and feels “professional”)
    ...
    if last_booking:
        ...
        # WhatsApp link (ONE per booking): use passenger 1 whatsapp if present
        default_phone = ""
        if pax_details and (pxd0 := pax_details[0]).get("whatsapp"):
            default_phone = pxd0.get("whatsapp", "")
